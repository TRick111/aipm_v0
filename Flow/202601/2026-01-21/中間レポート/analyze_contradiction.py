# -*- coding: utf-8 -*-
"""
4.2の客単価上昇と6.4のHigh/Low分析の矛盾を解明する分析
"""
...
def analyze_high_low_by_period(df_period, period_name):
    ...
    # 日次集計
    daily = df_dinner.groupby(['H.集計対象営業年月日']).agg({
        'H.伝票番号': 'nunique',
        'H.小計': lambda x: x.drop_duplicates().sum(),
        'H.客数（合計）': lambda x: df_dinner[df_dinner['H.集計対象営業年月日'] == x.name].groupby('H.伝票番号')['H.客数（合計）'].first().sum()
    }).reset_index()
    ...
    return {
        ...
        'overall_median_price': daily['客単価'].median()
    }
...
1. 「4.2の客単価上昇」と「6.4のHigh/Low分析」は**異なる比較**をしている

   - 4.2: 期間A（5-7月）と期間B（10-12月）の**時系列比較**
     → 期間Aの全日の中央値 vs 期間Bの全日の中央値
     → 結果: {med_a:.0f}円 → {med_b:.0f}円 = {med_b - med_a:+.0f}円上昇

   - 6.4: 4月以降の**同一期間内**での売上High日 vs Low日の比較
...
2. なぜ6.4では客単価の差がほぼないのか？

   【仮説の検証】
   - 各期間の中央値: 4月={result_april['overall_median_price']:.0f}円, 期間A={result_a['overall_median_price']:.0f}円, 期間B={result_b['overall_median_price']:.0f}円
   - 4月以降全体の中央値: {result_all['overall_median_price']:.0f}円

   → 4月の客単価が他の月と比べて低い/高いか確認すると:
     4月: {result_april['overall_median_price']:.0f}円
     期間A: {result_a['overall_median_price']:.0f}円
     期間B: {result_b['overall_median_price']:.0f}円

3. 結論

   「4.2の客単価上昇」は**時間経過に伴う変化**を見ている
   「6.4のHigh/Low分析」は**同一時点での売上を決める要因**を見ている

   - 時間経過とともに客単価は上昇している（メニュー改定、価格戦略の効果）
   - しかし、**同じ日の中で売上が高いか低いかを決めるのは「客数」であり「客単価」ではない**
   
   つまり:
   - 長期トレンド: 客単価は上昇している ✓
   - 日次変動: 売上の高低を決めるのは客数であり、客単価はほぼ一定 ✓

   これらは矛盾ではなく、**異なるレベルの分析**である。
""")

print("\n" + "=" * 80)
print("分析4: 各期間内でのHigh/Low比較の詳細")
print("=" * 80)

print(f"""
【各期間ごとのHigh/Low比較（ディナー帯）】
...
- しかし、期間ごとの全体中央値は上昇している
- つまり、「日ごとの売上変動」と「長期トレンド」は異なるメカニズム
""")

print("\n" + "=" * 80)
print("分析5: ランチ帯でも同様か確認")
print("=" * 80)
...
def analyze_high_low_lunch(df_period, period_name):
    ...
    daily = df_lunch.groupby(['H.集計対象営業年月日']).agg({
        'H.伝票番号': 'nunique',
        'H.小計': lambda x: x.drop_duplicates().sum(),
        'H.客数（合計）': lambda x: df_lunch[df_lunch['H.集計対象営業年月日'] == x.name].groupby('H.伝票番号')['H.客数（合計）'].first().sum()
    }).reset_index()
    daily.columns = ['日付', '組数', '売上', '客数']
    daily = daily[daily['客数'] > 0].copy()
    daily['客単価'] = daily['売上'] / daily['客数']
