...
def main():
    ...
    CHANNELS = {"EAT IN", "TAKE OUT"}

    for row in rows:
        category1 = row.get("D.商品カテゴリ1", "") or ""
        category2 = row.get("D.商品カテゴリ2", "") or ""

        # データによっては「カテゴリ1/2」が逆転しているため、チャネルが必ずカテゴリ1に入るよう正規化する
        if category1 not in CHANNELS and category2 in CHANNELS:
            category1, category2 = category2, category1
            row["D.商品カテゴリ1"] = category1
            row["D.商品カテゴリ2"] = category2
