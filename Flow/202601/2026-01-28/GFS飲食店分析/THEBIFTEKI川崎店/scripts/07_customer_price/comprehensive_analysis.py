...
print("=" * 80)
print(f"{STORE_NAME} 客単価上昇要因の包括的分析")
print("=" * 80)
...
# - 期間A: 2025年1-3月
# - 期間B: 2025年10-12月
period_a_start = pd.to_datetime('2025-01-01')
period_a_end = pd.to_datetime('2025-03-31')
...
result_a = calc_decomposition(df_period_a, '期間A (1-3月)')
...
ax1.hist(receipt_a['客単価'], bins=bins, alpha=0.5, label='期間A (1-3月)', color='steelblue', edgecolor='black')
...
ax2.plot(sorted_a, cumulative_a, linewidth=2, label='期間A (1-3月)', color='steelblue')
...
width = 0.35
ax3.bar(x - width/2, percentile_values_a, width, label='期間A (1-3月)', color='steelblue', alpha=0.7)
...
width = 0.35
ax1.bar(x - width/2, weekday_comp['平均客単価_A'], width, label='期間A (1-3月)', color='steelblue', alpha=0.7)
...
# 時間帯別客単価
ax3 = axes[1, 0]
ax3.plot(hour_comp['時間'], hour_comp['平均客単価_A'], 'o-', linewidth=2, markersize=6, label='期間A (1-3月)', color='steelblue')
...
# 時間帯別伝票数
ax4 = axes[1, 1]
ax4.bar(hour_comp['時間'], hour_comp['伝票数_A'], alpha=0.5, label='期間A (1-3月)', color='steelblue')
