...
exec_concl = []
exec_concl.append(f"<strong>当月POS売上は{fmt_yen(apr['sales'])}</strong>（{bd_apr}営業日 / {apr['visits']}会計 / 客数{apr['customers']}人）。日平均は<strong>{fmt_yen(sales_per_day_apr)}</strong>で前月比<strong>{fmt_diff(mom_per_day)}</strong>。客数 {fmt_diff(mom_customers)} の伸びを客単価 {fmt_diff(mom_per_person)} の低下が一部相殺、<strong>主因は{driver}</strong>（営業日換算で客数寄与{fmt_yen(visits_contrib)} / 単価寄与{fmt_yen(unit_contrib)}）")
exec_concl.append(f"<strong>4月PL: 売上{fmt_yen(pl_apr_sales)} / 計上費用{fmt_yen(pl_apr_exp)} (食材・ドリンク・広告費・家賃・備品・管理費)</strong>。1-2月実績の人件費 (¥347〜414k) / 水道光熱費 (¥80〜92k) / ローン返済 (¥169k) を加算した実態費用は<strong>¥3.0M規模</strong>。予算到達率{(budget_achievement*100):.1f}%、CLP人件費を含めた最終利益は<strong>▲¥800k〜▲¥1Mレンジ</strong>")
exec_concl.append(f"<strong>コース利用率{(apr['course_share']*100):.1f}%</strong> (3月10.1%) は依然低水準 / 1組人数{apr['avg_party_size']:.2f}人 (3月{mar['avg_party_size']:.2f}) で団体化進行 → <strong>客数増は団体・大人数化が牽引、客単価低下はコース機能不全と高額単品の伸び鈍化</strong>")

next_decisions = [
    "コース提案フローの強化 — 4月コース利用3.0% (5件) は3月10.1% (19件) から半減水準。価格訴求/提案動線/メニュー構成の3点を月内に再設計する",
    "5月集客強化策の選定 — 4月日平均¥116kから予算ライン¥113kは達成済も、CLP込み利益確保には日平均¥150k必要。SNS/Google/予約導線の中で5月単月に効く1-2本を特定する",
    "リニューアル後のレシピ補完 — 4月にPOS出現するが商品リストに無い品目を商品マスタに追加登録 (区分2a)",
]
...
<ul class="bl">
<li><strong>会計件数</strong>: {mar['visits']} → {apr['visits']} ({fmt_diff(mom_visits)}) = <strong>{apr['visits']-mar['visits']:+d}件</strong></li>
<li><strong>客数</strong>: {mar['customers']} → {apr['customers']}人 ({fmt_diff(mom_customers)}) = <strong>+{apr['customers']-mar['customers']}人</strong></li>
...
<li><strong>コース率</strong>: 10.1% → <strong>{(apr['course_share']*100):.1f}%</strong></li>
</ul>
<div class="info-box">団体化(1組+{(apr['avg_party_size']-mar['avg_party_size']):.2f}人)による客数増を、コース機能不全による1人単価低下が一部相殺した構造</div>
</div>
</div>
</div>
</div>''')
...
<div class="msg-bar">月内ベスト日が団体貸切で売上の山を作るも、コース利用が3月19件→4月{seg['course']['visits']}件へ低下</div>
<div class="slide-body">
<div class="two-col">
<div class="col-half">
...
<div class="warn-box" style="margin-top:8px">コース{seg['course']['visits']}件・{fmt_yen(seg['course']['sales'])}は3月19件・¥626kから低下 (会計件数 {seg['course']['visits']-seg_m['course']['visits']:+d} / 売上 {fmt_yen(seg['course']['sales']-seg_m['course']['sales'])})。<strong>コース提案ファネルの再構築が論点</strong> (第9章 論点1)</div>
</div>
</div>
</div>
</div>''')
...
<div class="msg-bar">当月評価: <strong>進展</strong> — 客数+12.1% / 1組人数増 / 看板商品が安定して定着、コース構造のみ依然弱い</div>
<div class="slide-body">
<div class="two-col">
<div class="col-half">
...
<div class="note-box" style="margin-top:8px"><strong>判定: 進展</strong>。看板（ハイボール・ジン）が安定定着、客数 +12.1% / 1組人数の増加で団体・大人数化のニーズに応えつつある。<strong>残課題はコース提案構造</strong>: 客単価押上げの中核装置として再構築が必要</div>
</div>
</div>
</div>
</div>''')
...
<div>4月コース利用率{(apr['course_share']*100):.1f}% (3月10.1%)。価格訴求／提案タイミング／メニュー構成の3点で <strong>「コース化が再構造的に成立する設計」</strong>を引くか、<strong>コース概念を諦めて単品ハイチケットに振る</strong>かを判断する。判断材料: 4月コース{seg['course']['visits']}件の中身分析、3月コース19件の中身分析、団体予約のコース化率。</div>
</div>
<div class="card">
