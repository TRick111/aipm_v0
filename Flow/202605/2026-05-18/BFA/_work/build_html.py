...
slides.append({
    ...
    "classification": [
        ...
        "PL系KPI（総費用／営業利益／FL率／人件費率）の出典: ダッシュボード本番DB",
    ],
    "kpis": [
        ...
        # PL系（v1.2 イテレーション7 追加・出典: ダッシュボード本番DB）
        ...
        {"label": "📊 営業利益率", "value": f"{D['pl']['op_profit_rate']:+.1f}%", "comp": [
            ("計算", "営業利益 ÷ 売上"),
            ("評価", "赤字" if D['pl']['op_profit_rate'] < 0 else "黒字"),
        ]},
    ...
    "caption": "売上は前年比で減少、PLは営業利益▲34万円の赤字。FL率35%・人件費率5%は業界平均より低水準。",
})
...
slides.append({
    ...
    "icon": "💰",
    "title": "PLサマリー（4月実績）",
    "subtitle": "売上はエアレジ／費用はダッシュボード本番DB",
    "message_icon": "⚠️",
    "message": f"4月は売上¥{m04['sales_total']:,.0f}に対して費用¥{PL['expense_total']:,.0f}と上回り、営業利益は¥{PL['op_profit']:,.0f}（{PL['op_profit_rate']:+.1f}%）の赤字に転落した。",
    "classification": [
        "売上: エアレジ会計明細（提供CSV）の月次総売上",
        "費用: ダッシュボード本番DBの9費目合計（家賃／人件費／食材／ドリンク／管理費／広告費／水道光熱費／備品／ローン返済）",
        "対象期間: 2026-04 月次集計",
    ],
    "big": {
        "label": "営業利益（売上 − 費用）",
    ...
    "caption": f"家賃（{PL['expense_breakdown'][0]['share']:.1f}%）が最大費目。ドリンク原価が次に大きく、固定費の重さが利益を圧迫している。",
})
...
slides.append({
    ...
    "y_label": "費用（円）",
    "message_icon": "⚠️",
    "message": f"総費用は1月¥2,352,939→2月¥2,832,178→4月¥3,368,379と増加傾向で推移し、家賃が固定費として最大の重しとなっている。",
    "classification": [
        "対象月: 1月・2月・4月（3月分はダッシュボードDBに未登録のため欠損）",
        "出典: ダッシュボード本番DB（store: bfa-001 / table: monthly_expenses）",
        "主要4費目のみ表示。残り5費目はPL-1スライド参照",
    ],
    "series": pl2_series,
    "highlight_last": True,
    "annotation": f"4月家賃 ¥{PL['trend'][-1]['exp_breakdown'].get('家賃',0):,.0f}",
    "caption": "家賃・ドリンク原価が大きく、構造的に高い固定費。3月分は本番DBに未登録のため欠損。",
})

# ───────────────────────────────────────────────
# PL-3: FL率・人件費率推移
# ───────────────────────────────────────────────
fl_data = [(t["month"], t["fl_rate"]) for t in PL["trend"] if t["fl_rate"] is not None and t["month"] != "2026-05"]
labor_data = [(t["month"], t["labor_rate"]) for t in PL["trend"] if t["labor_rate"] is not None and t["month"] != "2026-05"]
slides.append({
    "type": "line_chart",
    "icon": "📊",
    "title": "FL率・人件費率の推移",
    ...
    "unit": "%",
    "message_icon": "💡",
    "message": f"4月のFL率は{PL['fl_rate']:.1f}%（業界平均60%）、人件費率は{PL['labor_rate']:.1f}%（業界平均30%）でいずれも業界水準を大きく下回り、原価管理は健全だが人員投入が極端に少ない。",
    "classification": [
        "FL率 = （食材 + ドリンク）÷ 売上（売上はエアレジ会計明細、費用はダッシュボード本番DB）",
        "人件費率 = 人件費 ÷ 売上",
        "売上分母はボトル含む月次総売上（エアレジ売上分析画面と一致）",
        "3月分は本番DBに未登録のため欠損",
    ],
    "series": [
        {"name": "FL率（食材+ドリンク÷売上）", "color": "#1B4D3E", "data": fl_data},
        {"name": "人件費率（人件費÷売上）", "color": "#C9A961", "data": labor_data, "dash": True},
    ],
    "highlight_last": True,
    "annotation": f"4月 FL{PL['fl_rate']:.1f}% / 人件費{PL['labor_rate']:.1f}%",
    "caption": "FL率は安定的に低く健全。人件費率は業界平均30%を大きく下回り、オーナー稼働比率の高さが推測される。",
})
...
slides.append({
    ...
    "title": "PLからのインサイト",
    "message_icon": "💡",
    "message": f"4月の赤字¥{abs(PL['op_profit']):,.0f}は売上低迷と固定費の重さが主因であり、家賃・ローン返済（合計¥{PL['expense_breakdown'][0]['amount']+next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='ローン返済'),0):,.0f}）が圧迫要因として最大である。",
    "classification": [
        "出典: ダッシュボード本番DB（4月PL確定値）",
        "業界平均比較は飲食業の一般水準（FL60%／人件費30%）を参考",
    ],
    "items": [
        f"営業利益は¥{PL['op_profit']:,.0f}（{PL['op_profit_rate']:+.1f}%）で赤字に転落。前年同月比で売上が-23.7%減少していることが直接の主因",
        f"家賃¥{PL['expense_breakdown'][0]['amount']:,.0f}（売上比{PL['expense_breakdown'][0]['share']:.1f}%）が最大費目で、固定費の重さが利益を圧迫",
        f"ドリンク原価¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='ドリンク'),0):,.0f}（売上比{next((e['share'] for e in PL['expense_breakdown'] if e['category']=='ドリンク'),0):.1f}%）。FL率35.0%は業界平均60%より低く原価管理は健全",
        f"人件費¥{PL['labor_cost']:,.0f}（売上比{PL['labor_rate']:.1f}%）。業界平均30%より遥かに低く、オーナー兼任での運営と推測される",
        f"管理費¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='管理費'),0):,.0f}と広告費¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='広告費'),0):,.0f}が増加傾向。広告費は集客回復への先行投資の可能性",
        "ローン返済¥278,105が継続的に発生。資金繰り上のキャッシュフロー圧迫要因",
    ],
    "caption": "赤字脱却には売上回復（前年水準への戻し）と固定費の見直しが両輪。広告費投入の費用対効果を継続追跡する。",
})
...
slides.append({
    ...
    "items": [
        ...
        f"【PL】人件費率 {PL['labor_rate']:.1f}% と業界平均30%を大幅下回り、オーナー過剰負担が継続している可能性",
    ],
...
slides.append({
    ...
    "rows": [
        ...
        ["高", "PL赤字脱却", "売上¥3,024,490→¥3,400,000復元 ＋ 広告費効果検証", "営業利益▲34万→±0へ"],
...
slides.append({
    ...
    "classification": [
        "売上データ: エアレジ会計明細（4月本体は提供CSVを正=¥3,024,490／比較はダッシュボード経由）",
        "PL費用データ: ダッシュボード本番DB（4月総費用¥3,368,379／3月分は未登録のため欠損）",
        "予算データ: gws CLI 経由 Google Drive「【BFA】本部サポート」PLシート（2026-04列未登録のため引用元未確定）",
    ],
    "items": [
        "対予算（消化率）: 「【BFA】本部サポート」PLシートに2026-04列が無く引用元セル未特定（要ユーザー確認）。",
        "ボトル購入は4月から計上開始（2件¥256,500）。提供CSV内で完結しており手動補正は不要。",
        "インバウンド（メモ「海外」）はエアレジ会計明細のメモ欄から抽出（30件）。CLP社員はメニュー名「CLP」部分一致（4件）。",
        "売上データ本体は週報パイプライン経由のダッシュボード本番DB（エアレジ会計明細）を使用",
        "営業日基準（朝6時境界・週次/日次/曜日/時間帯で使用）と売上日時基準（月売上で使用）の二軸を使い分け。",
        "客単価2系統: 経営判断用（売上−ボトル÷客数=¥4,740）／エアメイト比較用（売上÷客数=¥5,179）。",
        "前年同月比較データ（2025-04）: 売上¥3,802,800／客数777人／営業日30日（エアレジ会計明細 より）",
        "PL費用データ: ダッシュボード本番DB由来。1月¥2,352,939／2月¥2,832,178／4月¥3,368,379を取得。3月分は未登録のため推移グラフから欠落。",
        "業界平均比較（FL率60%／人件費率30%）は飲食業の一般水準を参考値として使用（具体的なベンチマークソースは未指定）。",
    ],
