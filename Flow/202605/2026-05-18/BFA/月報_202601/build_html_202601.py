...
_TARGET = _tm                  # "2026-01"
_PREV   = _prev_month_str(_tm) # "2025-12"
_PREV_Y = _prev_year_month_str(_tm) # "2025-01"
m04 = M[_TARGET]
m03 = M.get(_PREV)
m04_py = M.get(_PREV_Y)
_MONTH_LABEL = f"{_m}月"  # ex: "1月"
_PREV_Y_LABEL = f"{int(_PREV_Y.split('-')[0])}年{int(_PREV_Y.split('-')[1])}月"  # ex: "2025年1月"
_PREV_LABEL = f"{int(_PREV.split('-')[0])}年{int(_PREV.split('-')[1])}月"  # ex: "2025年12月"
...
slides.append({
    ...
    "classification": [
        ...
        "PL系KPI（総費用／営業利益／FL率／人件費率）は【BFA】本部サポート / PLシートに対象月のデータがないため非表示",
    ] if not _PL_OK else [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時（エアレジ売上分析画面と一致）",
...
slides.append({
    ...
    "y_label": "売上（円）",
    "message_icon": "⚠️",
    "message": f"{_MONTH_LABEL}の売上は{yen(m04['sales_total'])}（前月比 {diff_pct(m04['sales_total'], m03['sales_total']) if m03 else '—'}／前年同月比 {diff_pct(m04['sales_total'], m04_py['sales_total']) if m04_py else '— = 前年同月データなし'}）。",
    "classification": [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時（エアレジ売上分析画面と一致）",
    ...
    "caption": f"直近6ヶ月の売上推移。{_MONTH_LABEL}の前月比は {diff_pct(m04['sales_total'], m03['sales_total']) if m03 else '—'}。",
})
...
slides.append({
    ...
    "unit": "人",
    "message_icon": "✅",
    "message": (f"客数は前月{m03['customers']}人から当月{m04['customers']}人（前月比 {diff_pct(m04['customers'], m03['customers'])}／前年同月比 {diff_pct(m04['customers'], m04_py['customers']) if m04_py else '— = 前年同月データなし'}）。" if m03 else f"客数は当月{m04['customers']}人（前年同月データなし）。"),
    "classification": [
    ...
    "caption": f"客数推移（直近6ヶ月）。{_MONTH_LABEL}の前月比は {diff_pct(m04['customers'], m03['customers']) if m03 else '—'}。",
})
...
slides.append({
    ...
    "y_label": "客単価（円）",
    "message_icon": "⚠️",
    "message": (f"客単価（ボトル除く）は前月{yen(m03['kyakutanka'])}から当月{yen(m04['kyakutanka'])}（前月比 {diff_pct(m04['kyakutanka'], m03['kyakutanka'])}／前年同月比 {diff_pct(m04['kyakutanka'], m04_py['kyakutanka']) if m04_py else '— = 前年同月データなし'}）。" if m03 else f"客単価は当月{yen(m04['kyakutanka'])}（前年同月データなし）。"),
    "classification": [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時",
    ...
    "caption": f"客単価推移（直近6ヶ月）。{_MONTH_LABEL}はボトル除く{yen(m04['kyakutanka'])} / 全部込み{yen(m04['kyakutanka_with_bottle'])}。",
})
...
slides.append({
    ...
    "icon": "📅",
    "title": f"前年同月比（{_PREV_Y_LABEL}との比較）",
    "message_icon": "⚠️" if m04_py else "ℹ️",
    "message": (f"売上は前年同月から{diff_pct(m04['sales_total'], m04_py['sales_total'])}（差額{yen(m04_py['sales_total']-m04['sales_total'])}）、客数は{diff_pct(m04['customers'], m04_py['customers'])}（差{m04_py['customers']-m04['customers']}人）。" if m04_py else f"前年同月（{_PREV_Y}）のデータが揃わないため、前年比はすべて『—』表示。前月比（{_PREV}）のみ参照。"),
    "classification": [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時",
    ...
    "caption": (f"{_PREV_Y_LABEL}対比サマリー。" if m04_py else f"前年同月（{_PREV_Y}）のデータが無いため前年比は出していない。前月比は別スライド参照。"),
})
...
slides.append({
    ...
    "message_icon": "✅",
    "message": f"{_MONTH_LABEL}のインバウンド客は{p11['customers']}人、売上構成比{p11['share_sales']:.1f}%。",
    "classification": [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時",
    ...
    "caption": f"{_MONTH_LABEL}のインバウンド客は{p11['customers']}人、売上構成比 {p11['share_sales']:.1f}%。",
})
...
slides.append({
    ...
    "icon": "🎉",
    "title": "客層分析③：イベント比率",
    "subtitle": "POSカテゴリ「イベント」",
    "message_icon": "✅" if p13['sales'] > 0 else "ℹ️",
    ...
    "caption": f"{_MONTH_LABEL}のイベント売上¥{p13['sales']:,.0f}（{p13['share_sales']:.1f}%）。{p13['accounts']}件で{p13['customers']}人を集客。",
})
...
slides.append({
    ...
    "message_icon": "ℹ️" if p14['sales'] == 0 else "💡",
    "message": (f"{_MONTH_LABEL}のボトル購入実績は¥0（該当取引なし）。" if p14['sales'] == 0 else f"{_MONTH_LABEL}のボトル販売は{p14['accounts']}件で¥{p14['sales']:,.0f}（月次売上構成比{p14['share_sales']:.1f}%）。"),
    ...
    "rows": [
        ...
        ["前月比較", yen(m03['bottle_sales']) if m03 else "—", f"前月（{_PREV}）のボトル売上"],
        ["日次・曜日・時間帯", "除外", "単位売上の傾向を歪めないため"],
    ],
    "caption": ("ボトル購入の実績なし（¥0）。月次総売上にのみ含めるルールは維持（対象月は加算ゼロ）。" if p14['sales'] == 0 else "ボトル購入は月次総売上にのみ含め、それ以外のすべての集計（客単価／週次／日次／曜日別／時間帯別）から除外。"),
})
...
slides.append({
    ...
    "caption": f"コース全体の構成比は {p15['share_sales']:.1f}%。うちその他CLPコース比率: {(p16['sales']/p15['sales']*100):.0f}%" if p15['sales'] else f"コース実績なし（{_MONTH_LABEL}は該当取引なし）。",
})
...
slides.append({
    ...
    "message_icon": "💡",
    "message": (f"その他CLPコースは{p16['accounts']}件・{p16['customers']}人で売上¥{p16['sales']:,.0f}（売上構成比{p16['share_sales']:.1f}%・1人あたり¥{p16['sales']/p16['customers']:,.0f}）。" if p16['customers'] else f"{_MONTH_LABEL}のその他CLPコース実績なし（該当取引なし）。"),
    ...
    "caption": (f"その他CLPコース: {p16['accounts']}件・{p16['customers']}人・売上¥{p16['sales']:,.0f}（{p16['share_sales']:.1f}%）。客単価¥{p16['sales']/p16['customers']:,.0f}/人。" if p16['customers'] else f"{_MONTH_LABEL}のその他CLPコース実績なし。"),
})
...
slides.append({
    ...
    "subtitle": "数量ベース",
    "message_icon": "✅",
    "message": (f"フード出数TOP3: " + "／".join(f"{r['menu_name']}({r['qty']:.0f}食・{r['customers']}人)" for r in food_qty[:3]) + "。") if food_qty else "フード出数データなし。",
    "classification": [
    ...
    "caption": (f"フード出数TOP3: " + "／".join(r['menu_name'] for r in food_qty[:3]) + f"。{RANK_CAPTION_NOTE}") if food_qty else f"フード出数データなし。{RANK_CAPTION_NOTE}",
})
...
slides.append({
    ...
    "subtitle": "数量ベース",
    "message_icon": "✅",
    "message": (f"ドリンク出数TOP3: " + "／".join(f"{r['menu_name']}({r['qty']:.0f}杯・{r['customers']}人)" for r in drink_qty[:3]) + "。") if drink_qty else "ドリンク出数データなし。",
    "classification": [
    ...
    "caption": (f"ドリンク出数TOP3: " + "／".join(r['menu_name'] for r in drink_qty[:3]) + f"。{RANK_CAPTION_NOTE}") if drink_qty else f"ドリンク出数データなし。{RANK_CAPTION_NOTE}",
})
...
slides.append({
    ...
    "subtitle": "売上ベース",
    "message_icon": "💡",
    "message": (f"フード売上TOP3: " + "／".join(f"{r['menu_name']}({yen(r['sales'])})" for r in food_sales[:3]) + "。") if food_sales else "フード売上データなし。",
    "classification": [
    ...
    "caption": (f"フード売上TOP3: " + "／".join(r['menu_name'] for r in food_sales[:3]) + f"。{RANK_CAPTION_NOTE}") if food_sales else f"フード売上データなし。{RANK_CAPTION_NOTE}",
})
...
slides.append({
    ...
    "subtitle": "売上ベース",
    "message_icon": "✅",
    "message": (f"ドリンク売上TOP3: " + "／".join(f"{r['menu_name']}({yen(r['sales'])}・{r['share']:.1f}%)" for r in drink_sales[:3]) + "。") if drink_sales else "ドリンク売上データなし。",
    "classification": [
    ...
    "caption": (f"ドリンク売上TOP3: " + "／".join(r['menu_name'] for r in drink_sales[:3]) + f"。{RANK_CAPTION_NOTE}") if drink_sales else f"ドリンク売上データなし。{RANK_CAPTION_NOTE}",
})
...
slides.append({
    ...
    "message_icon": "💡",
    "message": f"{_MONTH_LABEL}の週別日平均売上は {max_w['year_week']} が{yen(max_w['daily_avg'])}/日と最高、{min_w['year_week']} が{yen(min_w['daily_avg'])}/日と最低。",
    "classification": [
...
slides.append({
    ...
    "classification": [
        "営業日基準（朝6時境界 ／ 0:00〜5:59 は前日扱い）",
        f"曜日 = 営業日付の曜日（{_MONTH_LABEL}: " + "／".join(f"{w['weekday']}{w['business_days']}日" for w in wd) + "）",
...
slides.append({
    ...
    "message_icon": "💡",
    "message": (f"{_MONTH_LABEL}の時間帯別データは抽出されていない（バケット別入店時刻が未集計）。" if not hr_sorted else f"{_MONTH_LABEL}の時間帯別構成: " + "／".join(f"{h['hour_bucket']}時 {h['share']:.1f}%" for h in hr_sorted) + "。"),
    "classification": [
        "入店時刻ベース（会計時間 − 滞在時間）",
    ...
    "table_rows": [[h["hour_bucket"]+"時", yen(h["sales_excl_bottle"]), f"{h['customers']}人", pctv(h["share"])]
                   for h in hr_sorted],
    "caption": ("時間帯別バケットの集計データが提供データに含まれていない。" if not hr_sorted else f"{_MONTH_LABEL}の時間帯別月間売上の内訳。"),
})
...
slides.append({
    ...
    "message_icon": "💡",
    "message": (f"カテゴリ別TOP3: " + "／".join(f"{c['category1']}({c['share']:.1f}%)" for c in cat[:3]) + "。") if cat else "カテゴリ別データなし。",
    "classification": [
        f"集計対象: {_y}/{_m}/1 0:00 〜 {_m}/{_last_day} 23:59 の売上日時",
    ...
    "caption": (f"{cat[0]['category1']}が{cat[0]['share']:.1f}%で最大。上位10カテゴリの内訳。" if cat else "カテゴリデータなし。"),
})
...
if _PL_OK:
    ...
    _PL_MSG = PL.get("unavailable_reason") or "【BFA】本部サポート / PLシート に対象月のデータがないため出力していません"
    for _t, _icon in [
...
if dr:
    slides.append({
        ...
        "message_icon": "💡",
        "message": f"{_MONTH_LABEL}の日報入力{len(dr)}件をもとに、来客特徴・改善ポイントを抽出。",
        "classification": [
        ...
        "reports": [{"date": r["営業日"], "name": r["入力者氏名"],
                    ...
                    for r in dr],
        "caption": f"{_MONTH_LABEL}の日報からの抜粋。",
    })
...
slides.append({
    ...
    "title": "今月の強み",
    "message_icon": "✅",
    "message": f"{_MONTH_LABEL}は構成比上位カテゴリ・好調曜日・主力ドリンクが収益の柱となった。",
    "classification": [
        "抽出基準: 構成比10%超、好調曜日（日平均最高）、出数/売上TOP3",
    ],
    "items": [
        (f"インバウンド客が{p11['customers']}人・売上¥{p11['sales']:,.0f}（売上構成比 {p11['share_sales']:.1f}%）" if p11['customers'] > 0 else f"インバウンド客の取り込み余地: {_MONTH_LABEL}は0件、Google検索経由の認知強化が必要"),
        f"コース利用（{p15['accounts']}件・{p15['customers']}人・¥{p15['sales']:,.0f}／{p15['share_sales']:.1f}%）が最大カテゴリの一つ",
        f"{max_wd['weekday']}曜日（日平均{yen(max_wd['daily_avg_sales'])}）が最強曜日",
    ] + ([f"{drink_qty[0]['menu_name']}（{drink_qty[0]['qty']:.0f}杯／{yen(drink_qty[0]['sales'])}）が主力ドリンク"] if drink_qty else []) + ([f"ボトル販売¥{p14['sales']:,.0f}（売上構成比{p14['share_sales']:.1f}%）"] if p14['sales'] > 0 else []),
    "caption": f"{_MONTH_LABEL}の収益柱（カテゴリ・曜日・主力商品）。",
})
...
slides.append({
    ...
    "title": "今月の課題",
    "message_icon": "⚠️",
    "message": f"{_MONTH_LABEL}は前月比・前年比でマイナス傾向のKPIと、弱い曜日が課題として顕在化。",
    "classification": [
        "抽出基準: 前年比 or 前月比でマイナス、曜日・時間帯で著しく低い水準、客単価低下セグメント",
    ],
    "items": [
        f"売上は前月比 {diff_pct(m04['sales_total'], m03['sales_total']) if m03 else '—'}／前年同月比 {diff_pct(m04['sales_total'], m04_py['sales_total']) if m04_py else '— = 前年同月データなし'}（{yen(m04['sales_total'])}）",
        f"客数は前月比 {diff_pct(m04['customers'], m03['customers']) if m03 else '—'}／前年同月比 {diff_pct(m04['customers'], m04_py['customers']) if m04_py else '— = 前年同月データなし'}（{m04['customers']}人）",
        (f"客単価が前月¥{m03['kyakutanka']:,.0f}から当月¥{m04['kyakutanka']:,.0f}（{diff_pct(m04['kyakutanka'], m03['kyakutanka'])}）" if m03 else f"客単価当月¥{m04['kyakutanka']:,.0f}"),
        f"{min_wd['weekday']}曜日が日平均¥{min_wd['daily_avg_sales']:,.0f}と最弱曜日",
    ] + ([f"【PL】営業利益 ¥{PL['op_profit']:,.0f}（{PL['op_profit_rate']:+.1f}%）" + ("（赤字）" if PL['op_profit'] < 0 else "")] if _PL_OK else ["【PL】PLシート未入力のためPL系課題は別途確認が必要"]),
    "caption": f"{_MONTH_LABEL}の主な課題（売上・客単価・弱曜日）。",
})
...
slides.append({
    ...
    "title": "次月以降の施策案",
    "message_icon": "💡",
    "message": "高優先は売上回復・客単価・弱曜日対策、中優先は新規収益柱の拡大策、低優先は運用体制の改善。",
    "classification": [
        "優先順位: 高=売上構造の即時改善、中=新規収益柱の拡大、低=運用体制の継続改善",
    ],
    "headers": ["優先", "テーマ", "施策案", "狙い・指標"],
    "rows": [
        ["高", "インバウンド", "Googleマップ口コミ強化（英語応答テンプレ／写真追加）", f"海外比率 {p11['share_sales']:.1f}% → 引き上げ"],
        ["高", "客単価回復", "コース後アラカルト追加の常時提案 ／ ペアリングメニュー導入", f"客単価 {yen(m04['kyakutanka'])} → 引き上げ"],
        ["高", "弱曜日強化", f"{min_wd['weekday']}曜・平日帯のテコ入れ（早割／限定メニュー）", f"{min_wd['weekday']}曜日平均 ¥{min_wd['daily_avg_sales']:,.0f} → 引き上げ"],
        ["中", "ボトル販売", "CLP社員向けボトルキープ施策の継続・定型化", f"月次 {yen(p14['sales'])} → 拡大"],
        ["中", "新規メニュー検証", "出数下位メニューの入替えで主力ローテーション維持", "出数TOP15の入替え率を月次計測"],
        ["中", "PL整備", "【BFA】本部サポート / PLシートの当該月入力", "PL系KPI（FL率・人件費率）の可視化"],
        ["低", "日報強化", f"{_MONTH_LABEL}の日報入力{len(dr)}件は少ない。週次入力リズム化", "月10〜15件入力"],
    ],
    "caption": "高優先3件は売上・客単価・弱曜日の即時改善、中優先3件は構造的な収益力強化・PL整備に向けた施策。",
})
...
<head>
<meta charset="UTF-8">
<title>__STORE__ 月次営業報告 — __TM__ (v1)</title>
