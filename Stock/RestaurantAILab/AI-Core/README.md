...
> **AI-Core 関連プロジェクト群（兄弟Project）**
> - `Stock/RestaurantAILab/AI-Core/` — 本プロジェクト（AIコア共通基盤・PL = 提供メニュー選択ツール）
> - `Stock/RestaurantAILab/AI-Core_飯武さん導入/` — 飯武さん向け導入支援（BL-0066, 2026-04-30 第1回）
> - `Stock/RestaurantAILab/AI-Core_共用リポジトリ/` — 共用リポ設計（BL-0054 着手後に新設予定）

## 背景
- AIOS / AI Core を授業・伴走・導入支援の形で複数クライアントへ展開するための共通母艦プログラム
- 個別案件（Pon・UNISON・西村・飯武）から再利用可能な提供資産（教材・テンプレート・契約書・運用基盤）を一般化して保守する
- 顧客別の導入支援は **兄弟Project**（`AI-Core_<顧客名>`）として独立させ、本プロジェクトは共通基盤＋PLを担う
