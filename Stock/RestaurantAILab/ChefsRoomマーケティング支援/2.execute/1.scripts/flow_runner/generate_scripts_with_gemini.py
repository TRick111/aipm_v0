...
def find_repo_root(start: Path) -> Path:
    """
    どこから実行しても repo root（Flow/ と Stock/ がある階層）を推定する。
    見つからない場合は start を返す。
    """
    for p in [start] + list(start.parents):
        if (p / "Flow").exists() and (p / "Stock").exists():
            return p
    return start


def guess_flow_date(start: Path) -> str:
    """
    Flow/YYYYMM/YYYY-MM-DD/... の YYYY-MM-DD を推定。
    見つからなければ空文字。
    """
    for p in [start] + list(start.parents):
...
# スクリプトのディレクトリ基準のパス設定（Flowでの実行をデフォルトにする）
SCRIPT_DIR = Path(__file__).resolve().parent
FLOW_DATE = guess_flow_date(SCRIPT_DIR)
OUT_BASENAME = f"generated_scripts_{FLOW_DATE.replace('-', '_')}.csv" if FLOW_DATE else "generated_scripts.csv"

# - 入力: Flow日付フォルダ直下の input.csv をデフォルト（= このファイルの1つ上の階層）
CSV_IN = os.getenv("CSV_IN", str((SCRIPT_DIR / "../input.csv").resolve()))
# - 出力: Flow配下に残す（このファイルの1つ上の階層の output/ に出す）
CSV_OUT = os.getenv("CSV_OUT", str((SCRIPT_DIR / f"output/{OUT_BASENAME}").resolve()))

# - prompt/examples は Flow 側にコピーしたファイルをデフォルトにする
PROMPT_MD = os.getenv("PROMPT_MD", str((SCRIPT_DIR / "prompt_improved.md").resolve()))
EXAMPLES_YAML = os.getenv("EXAMPLES_YAML", str((SCRIPT_DIR / "examples.yaml").resolve()))

# 出力CSVのカラム（仕様準拠）
OUTPUT_HEADERS = ["No", "テーマ"] + [f"シーン{i}" for i in range(1, 18)] + ["参考例ID"]

# Gemini 準備（scripts/.env を明示的に読み込む）
# プロジェクトルート: .../aipm_v0（推定）
...
def main() -> int:
    ...
    if not path_in.exists():
        ...
        return 1
    if not Path(PROMPT_MD).exists():
        print(f"prompt_improved.md が見つかりません: {Path(PROMPT_MD)}")
        return 1
    if not examples_yaml.exists():
        print(f"examples.yaml が見つかりません: {examples_yaml}")
        return 1
