...
def main():
    ...
    weekend_records = [r for r in records if not r["is_weekday"]]
    
    datasets = [
        ("平日", weekday_records, "#2196F3"),
        ("土日", weekend_records, "#E53935")
    ]
    
    # 3. 各データセットで散布図を作成
    for dataset_name, data_records, color in datasets:
        if not data_records:
            print(f"{dataset_name}: データなし、スキップ")
            continue
        
        print(f"\n{'=' * 70}")
        print(f"売上との相関係数（{dataset_name}）")
        print("=" * 70)
        
        sales = [r["sales"] for r in data_records]
        
        factors = [
            ("総来店組数", [r["total_groups"] for r in data_records]),
            ("総来店客数", [r["total_customers"] for r in data_records]),
            ("ランチ組数", [r["lunch_groups"] for r in data_records]),
            ("ディナー組数", [r["dinner_groups"] for r in data_records]),
            ("ディナー比率", [r["dinner_ratio"] for r in data_records]),
            ("平均滞在時間", [r["avg_duration"] for r in data_records]),
            ("ピーク店内人数", [r["peak_occupancy"] for r in data_records]),
            ("組単価", [r["avg_spend_per_group"] for r in data_records]),
            ("客単価", [r["avg_spend_per_customer"] for r in data_records]),
        ]
        ...
        for idx, (name, corr, values) in enumerate(top_factors):
            ...
            # 散布図
            ax.scatter(values, sales, alpha=0.5, s=40, c=color)
            
            # 回帰直線
            z = np.polyfit(values, sales, 1)
            x_line = np.linspace(min(values), max(values), 100)
            ax.plot(x_line, z[0]*x_line + z[1], 'k--', linewidth=2)
        ...
        plt.suptitle(f'売上と相関する要因（{dataset_name}、相関の高い順）', fontsize=14, y=1.02)
        plt.tight_layout()
        
        filename = f"sales_factor_exploration_{dataset_name}.png"
        ...
        high_sales = [r for r in data_records if r["sales"] > median_sales]
        low_sales = [r for r in data_records if r["sales"] <= median_sales]
        ...
        plt.suptitle(f'売上上位/下位の2軸分布（{dataset_name}）', fontsize=14)
        plt.tight_layout()
        
        filename2 = f"sales_2d_separation_{dataset_name}.png"
