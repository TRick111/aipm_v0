...
# --------------------------------------------
# CLI引数（互換維持）
# - 引数なし: 従来通り（相対パス/カレント出力）
# - 引数あり: store-root配下に出力できるようにする
# --------------------------------------------
...
def _resolve_io():
    if not (args.store_root or args.input or args.out_dir):
        return Path("../transformed_pos_data_eatin.csv")
    ...
    out_dir = Path(args.out_dir) if args.out_dir else store_root / "Y2Y"
    out_dir.mkdir(parents=True, exist_ok=True)
    os.chdir(out_dir)
    return input_path


_INPUT_PATH = _resolve_io()
_OUT_DIR = Path(os.getcwd()).resolve()
_ASSETS_DIR = Path(args.assets_dir).resolve() if args.assets_dir else (_OUT_DIR / "_slide_assets")
...
def _split_4rows(img_path: Path, names: list[str]):
    ...
    for box, name in zip(boxes, names):
        img.crop(box).save(_ASSETS_DIR / name)
