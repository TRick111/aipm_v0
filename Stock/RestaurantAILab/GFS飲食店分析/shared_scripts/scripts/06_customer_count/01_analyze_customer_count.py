...
# japanize_matplotlib は環境に無い場合があるため任意依存にする
try:
    import japanize_matplotlib  # type: ignore
except Exception:
    japanize_matplotlib = None  # noqa: F401
...
if args.store_root or args.input:
    store_root = Path(args.store_root or ".").resolve()
    OUTPUT_DIR = store_root / "CustomerCountAnalysis"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    DATA_PATH = Path(args.input) if args.input else store_root / "data/intermediate/transformed_pos_data_eatin.csv"
else:
...
# 空でも downstream が落ちないように、ヘッダーだけは必ず出す
expected_cols = [
    '月',
    f'{year1}年_営業日当たり客数',
    f'{year2}年_営業日当たり客数',
...
for c in expected_cols:
    if c not in factor_summary.columns:
        factor_summary[c] = []
factor_summary = factor_summary[expected_cols]
