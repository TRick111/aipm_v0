...
parser.add_argument("--assets-dir", default=None, help="分割素材の出力先（未指定なら <store-root>/reports/_slide_assets/）")
...
if args.store_root or args.input or args.out_dir:
    ...
    OUTPUT_DIR = str(Path(args.out_dir).resolve() if args.out_dir else store_root / "SegmentAnalysis/HighLowAnalysis")
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    INPUT_CSV = str(Path(args.input) if args.input else store_root / "data/intermediate/transformed_pos_data_eatin.csv")
else:
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, 'HighLowAnalysis')
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    INPUT_CSV = os.path.join(os.path.dirname(SCRIPT_DIR), 'transformed_pos_data_eatin.csv')
...
if args.split_assets:
    if args.assets_dir:
        ...
        ASSETS_DIR = Path(args.store_root).resolve() / "reports" / "_slide_assets"
    else:
        ASSETS_DIR = Path(OUTPUT_DIR).resolve().parent / "_slide_assets"
...
if args.split_assets:
    _split_2x3_cols(Path(OUTPUT_DIR) / "01_high_low_distribution.png")
