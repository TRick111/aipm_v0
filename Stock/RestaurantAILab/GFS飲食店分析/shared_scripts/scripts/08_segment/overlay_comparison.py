...
parser.add_argument("--period-filter", default="all", choices=["all", "after", "before"])
args, _ = parser.parse_known_args()

if args.store_root or args.out_dir:
    store_root = Path(args.store_root or ".").resolve()
    OUTPUT_DIR = str(Path(args.out_dir).resolve() if args.out_dir else store_root / "SegmentAnalysis")
...
suffix = ""
if args.period_filter == "after":
    daily = daily[daily["segment"].astype(str).str.startswith("After_")].copy()
    segments = ["After_Lunch", "After_Dinner"]
    suffix = "_after"
elif args.period_filter == "before":
    daily = daily[daily["segment"].astype(str).str.startswith("Before_")].copy()
    segments = ["Before_Lunch", "Before_Dinner"]
    suffix = "_before"
else:
...
title_suffix = "（4月以降のみ）" if args.period_filter == "after" else ("（3月以前のみ）" if args.period_filter == "before" else "")
plt.suptitle(f'セグメント比較（重ね表示）{title_suffix}', fontsize=16, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/09_overlay_all{suffix}.png', dpi=150, bbox_inches='tight')
plt.close()
print(f"  保存: 09_overlay_all{suffix}.png")
