...
_next_y = _y + 1 if _m == 12 else _y
_PERIOD_DASH = f"{_y}年{_m}月の業務日（朝6時境界、深夜営業を当日扱い）"
_PERIOD_JP = f"{_y}年{_m}月の業務日（{_m}/1 6:00 〜 {_next_m}/1 5:59 JST、朝6時境界）"
_PERIOD_JP_HEADER = f"{_y}年{_m}月（業務日: {_m}/1 〜 {_m}/{_last_day}）"
...
MONTHLY_SALES_NOTE = f"※月売上の集計対象: {_PERIOD_JP}（エアレジ売上分析画面と一致）。月次／週次／日次／曜日別／時間帯別すべて業務日基準で統一。"
...
# ═══════════════════════════════════════════════
# PL分析セクション（v1.2 イテレーション7-9 ／ 日報の前に配置）
# 出典: ダッシュボードDB
# ═══════════════════════════════════════════════
PL = D["pl"]
...
if _PL_AVAILABLE: slides.append({
    ...
    "classification": [
        "売上の出典: エアレジ会計明細（提供CSV）の月次総売上（ボトル含む）",
        "費用の出典: ダッシュボードDB（9費目: 家賃／人件費／食材／ドリンク／管理費／広告費／水道光熱費／備品／ローン返済）",
        "対象期間: 2026-04 月次集計",
    ],
    "big": {
        "label": "営業利益（売上 − 費用）",
...
if _PL_AVAILABLE: slides.append({
    ...
    "classification": [
        "対象月: 1月・2月・4月（2026年3月のPLデータは、【BFA】本部サポート / PLシート にデータがないため出力していません）",
        "出典: 4月はダッシュボードDB／1〜2月は【BFA】本部サポート / PLシート",
        "主要4費目のみ表示。残り5費目はPL-1スライド参照",
    ],
    "series": pl2_series,
    "highlight_last": True,
    "annotation": f"4月家賃 ¥{PL['trend'][-1]['exp_breakdown'].get('家賃',0):,.0f}",
    "caption": "家賃・ドリンク原価が大きく、構造的に高い固定費。3月分は【BFA】本部サポート / PLシートにデータがないため非表示。",
})
...
if _PL_AVAILABLE: slides.append({
    ...
    "title": "PLからのインサイト",
    "message_icon": "💡",
    "message": f"4月の赤字¥{abs(PL['op_profit']):,.0f}は売上低迷と固定費の重さが主因であり、家賃・ローン返済（合計¥{PL['expense_breakdown'][0]['amount']+next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='ローン返済'),0):,.0f}）が最大の圧迫要因となっている。",
    "classification": [
        "出典: ダッシュボードDB（4月PL確定値）",
        "FL率の分母 = 月次総売上（ボトル含む）",
        "目安は飲食業の一般的な水準（FL率60%／人件費率30%）を参考",
    ],
    "items": [
        ...
        f"ドリンク原価¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='ドリンク'),0):,.0f}（売上比{next((e['share'] for e in PL['expense_breakdown'] if e['category']=='ドリンク'),0):.1f}%）。FL率{PL['fl_rate']:.1f}%は目安60%より低く原価管理は健全",
        f"人件費¥{PL['labor_cost']:,.0f}（売上比{PL['labor_rate']:.1f}%）。目安30%より遥かに低い水準",
        f"管理費¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='管理費'),0):,.0f}・広告費¥{next((e['amount'] for e in PL['expense_breakdown'] if e['category']=='広告費'),0):,.0f}を計上",
        "ローン返済¥278,105を計上",
    ],
    "caption": "赤字脱却には売上回復と固定費の見直しが両輪。各費目の継続モニタリングが必要。",
})
