...
- 集計軸の使い分け:
  * すべて業務日基準（朝6時境界 ／ 0:00〜5:59 を前日扱い）に統一
  * 月次総売上もエアレジ売上分析画面も業務日基準のため一致する
  * 旧仕様（月次のみ売上日時ベース）は 2026-05-18 廃止
- ボトル購入:
  * 月次総売上に含む
...
def calc_monthly_kpi(hdr, det):
    ...
    [2026-05-18 修正 / 組数+1問題 対応]
    - 組数(accounts) / 客数(customers) は ¥0会計（No charge全額無料）と
      赤伝（合計<¥0）を除外して「合計>¥0 の会計」だけをカウントする。
      → エアメイト集計と一致させるため。
    - 売上総額(sales_total) は従来通り全件合算（赤伝は売上から正しく引かれる）。
    - ボトル系・客単価分母も「合計>0」フィルタを通す。
    """
    ...
    bottle_hdr_paid = bottle_hdr[bottle_hdr["合計"] > 0]
    bottle_customers = int(bottle_hdr_paid["人数"].sum())

    # 売上総額: 全件合算（赤伝のマイナス計上を尊重）
    total = float(hdr["合計"].sum())

    # 組数 / 客数: 合計>0 の会計だけ
    hdr_paid = hdr[hdr["合計"] > 0]
    excluded_zero = int((hdr["合計"] == 0).sum())
    excluded_neg = int((hdr["合計"] < 0).sum())
    ...
    return {
        ...
        "bottle_accounts": len(bottle_hdr_paid),
        "bottle_customers": bottle_customers,
        "sales_excl_bottle": excl_bottle,
        "customers": customers,
        "accounts": accounts,
        "accounts_raw": len(hdr),
        "accounts_excluded_zero_total": excluded_zero,
        "accounts_excluded_negative_total": excluded_neg,
...
def kpi_from_rawdata(rawdata_path, month_str):
    ...
    # 組数 / 客数 は合計>0 の会計だけ（組数+1問題対応 / 2026-05-18）
    acct_paid = acct[acct["total"] > 0]
    customers = int(acct_paid["customers"].sum())
    ...
    return {
        ...
        "sales_excl_bottle": excl_bottle,
        "customers": customers,
        "accounts": len(acct_paid),
        "accounts_raw": len(acct),
...
def main():
    ...
    # 3) Bottle 除外 ＋ 合計>0 フィルタ（組数+1問題対応 / 2026-05-18）
    bottle_ids = set(kpi["bottle_account_ids"])
    hdr_no_bottle = hdr[(~hdr["取引No"].isin(bottle_ids)) & (hdr["合計"] > 0)].copy()
