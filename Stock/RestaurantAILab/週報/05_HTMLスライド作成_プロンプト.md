...
| **主張バー（msg-bar）** | `27px` | `700` | **各スライドの核心メッセージ（1文）。ヘッダー直下に配置** |
...
| セクション見出し | `26px` | `800` | セクション小見出し |
| 本文テキスト | `20px` | `400` | 説明文、コメント、箇条書き |
| 表セル | `20px` | `400` | テーブルのデータ（`.dt` クラス） |
| 表ヘッダー | `19px` | `700` | テーブルのヘッダー行 |
| 施策一覧表 | `18px` | `400` | 施策表のデータ（`.at` クラス） |
| 施策一覧ヘッダー | `16px` | `700` | 施策表のヘッダー行 |
| 注釈・キャプション | `14px` | `400` | グラフ軸ラベル、出典 |
| **アノテーション** | `16px` 〜 `20px` | `700` | グラフ上の吹き出し内テキスト |
...
#### 3.3 主張バー（msg-bar）— 全スライド共通UI（最重要）

**タイトルスライド以外のすべてのスライドに、ヘッダー帯の直下に「主張バー」を配置すること。**
...
**注意**:
- メッセージが長すぎてスライドに収まらない場合は、核心部分を短縮して1〜2行以内に収めること
