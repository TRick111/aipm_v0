...
- **共通ルール（必読）**: `./共通ルール.md`（業務日基準・主張バー・分類条件帯・フッター・絵文字・色だけで意味を運ばない・達成未達成・日本語表記・円グラフ仕様）
- **BFA固有ルール（BFAの場合のみ必読）**: `../共通/BFA固有ルール.md`（客単価2系統表示・抽出キー・インバウンド運用）
- **必須入力**: `{スライド構成}`（Markdownのスライド構成案。全スライドのタイトル・データ表・メッセージが含まれる）
- **グラフライブラリ**: D3.js v7（CDN: `https://d3js.org/d3.v7.min.js`）

---

## 0. スライド共通の構成（共通ルール §6 準拠）

タイトルスライド以外のすべてのスライドで、以下を上から順に配置する：

1. **ヘッダー帯**（`.slide-header` / タイトル + ページ番号 + プログレスバー）
2. **主張バー**（`.msg-bar` / オレンジ縦線×薄黄背景・28px・1〜2文・✅⚠️💡）— 既存実装と同じ
3. **分類条件帯**（`.classification-bar` / 緑薄色背景の `<ul><li>...</li></ul>` 箇条書き）— **新規追加。共通ルール §6.4 参照**
4. **本体**（KPIカード／グラフ／表 等）
5. **算出根拠フッター**（`.footer-cite` / 出典 + 集計対象）— **新規追加。共通ルール §6.5 参照**

```css
/* 分類条件帯（新規） */
.classification-bar {
  background: #EFF6F0;            /* 緑系薄色 */
  border-left: 4px solid #2D6B4F; /* 暗緑左ボーダー */
  padding: 8px 32px;
  font-size: 16px;
  color: #1F2A28;
}
.classification-bar ul { margin: 4px 0; padding-left: 20px; }
.classification-bar li { line-height: 1.6; }
...
<div class="msg-bar">⚠️ 7日中6日が4週平均を下回り、水曜日の客数要因が最大の不振原因。</div>
<div class="classification-bar">
  <ul>
    <li>集計対象: 2026-W06 の業務日（2026/02/02 6:00 〜 2026/02/09 5:59 JST、朝6時境界）</li>
    <li>比較対象: 直前4週間の同曜日平均</li>
    <li>ボトル購入は除外（BFA）</li>
  </ul>
</div>
<div class="slide-body">...</div>
<div class="footer-cite">出典: エアレジ会計明細 ／ 集計対象: 2026-W06 の業務日（朝6時境界）</div>
```
...
> **マスター判断（2026-05-19 / K-1 確定）**: 週報は **以下の現行パレットを公式採用** とする。月報のダークグリーン基調（`#1B4D3E`）への統一は将来検討事項であり、当面は本パレットを維持する。新規追加スライド（分類条件帯・主張バー周辺・カードボーダー等）では緑系のアクセントも許容するが、メインカラーは変更しない。
...
- [ ] **共通ルール §6 準拠**: 全スライド（タイトル除く）に主張バー＋分類条件帯（箇条書き）＋算出根拠フッターが配置されている
- [ ] **共通ルール §5**: ランキングは TOP15（フード／ドリンク 各）で構成、★マーク（平均販売価格±5%乖離）が付与されている
- [ ] **共通ルール §9**: 「達成」「未達」「達成率」は予算ありの場合のみ。前週比/4週平均比/前年同週比の場合は「+X%」「-X%」表記
- [ ] **共通ルール §10**: カタカナ英語を避け日本語で記述
- [ ] **共通ルール §12**: 1スライド3個までの絵文字、装飾目的のスパムなし
- [ ] **共通ルール §13**: 比率を扱うスライドにドーナツチャートを併設（180-200px / 中央% / explode / リーダーライン / 凡例）
- [ ] **BFA の場合**: 客単価が2系統（ボトル除く・全部込み）併記、`BFA固有ルール.md §B-2` 準拠
- [ ] **BFA の場合**: インバウンド（BFA-1）スライドは対象週終了日が 2026-04 以降のときのみ表示（`BFA固有ルール.md §B-7`）
- [ ] **BFA の場合**: 出典フッターは「エアレジ会計明細」と統一
