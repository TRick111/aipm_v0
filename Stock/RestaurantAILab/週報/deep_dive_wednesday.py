...
wed = df[df['business_date'] == pd.Timestamp('2025-12-31').date()].copy()
wed['hour'] = wed['entry_at_jst'].dt.hour

print('='*60)
print('水曜日 2025-12-31 (最も好調な日) - 詳細分析')
print('='*60)
print()

# Basic info
print('【基本情報】')
accounts = wed.groupby('account_id').first().reset_index()
...
print('【2. 時間帯バケット分析】')
accounts_time = wed.groupby('account_id').agg({
    'entry_at_jst': 'first',
    'account_total': 'first',
...
print('【4. 商品ミックス分析】')
print('  カテゴリ別売上構成:')
category_sales = wed.groupby('category1').agg({
    'price': 'sum',
    'menu_name': 'count'
}).reset_index()
...
print('  高単価商品 (¥1,500以上):')
high_price = wed[wed['price'] >= 1500].copy()
