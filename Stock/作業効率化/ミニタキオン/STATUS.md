...
status: in_progress
owner_turn: ai
updated_at: 2026-04-26T19:25:00+09:00
updated_by: master-agent
current_bl: BL-TBD-011
next_action: "BL-TBD-011 (Phase 4a API + CLI) Cockpit task f285aa69 で実装中。完了後 BL-TBD-012 (prompt 移行) → 本格 dogfood で drift 解消検証"
blocker: null
related_bls: [BL-TBD-001, BL-TBD-002, BL-TBD-003, BL-TBD-004, BL-TBD-005, BL-TBD-006, BL-TBD-007, BL-TBD-008, BL-TBD-009, BL-TBD-010, BL-TBD-011, BL-TBD-012]
tech_stack:
...
## 🎯 次のアクション
**Phase 4 (API + CLI 化) 進行中** — エージェントを「rule-following」から「skill-calling」へ転換。BL-TBD-011 (Cockpit task f285aa69) で API + `mt` CLI 実装中。完了後 BL-TBD-012 で prompt 移行 → 本格 dogfood。
...
- [ ] **BL-TBD-004 Phase 3a: ☀️ 朝の整理** — 実装完了、4/26 朝の実機テスト待ち (Cockpit task 83948eeb 完了)
  - ✅ UI ボタン / 「📅 今日の運用」セクション / Server Action / プロンプト v2 (305行) / vitest 47 pass / launchd 本番化

- [x] BL-TBD-008 Phase 3a v3: 運用フィードバック修正 (a4adcf48) — 完了報告済、田中さん再 dogfood 後に completed マーク予定

- [x] BL-TBD-005 Phase 3b: 🌙 夜の振り返り (976dc72a) — 完了報告済、田中さん再 dogfood 後 completed マーク予定
- [x] BL-TBD-009 ミニタキオン UI 拡張: 中央 Backlog.md 読込 (b0c9e231) — 完了報告済、田中さんが「ホームが情報過多」とフィードバック → BL-TBD-010 で改善

- [x] BL-TBD-010 ホーム「今日の選択」フィルタ + /backlog 別ページ (81100e18) — 完了報告済 (vitest 112 pass)、田中さん dogfood 中

- [x] BL-TBD-011 Phase 4a: API + CLI (f285aa69) — 完了報告済 (16 routes / mt CLI / vitest 148 pass / docs/API.md+CLI.md)
- [ ] **BL-TBD-012 Phase 4b: prompt の skill 化移行** — Cockpit task 4f55e433 で実装中 (2026-04-26 19:37 開始)
  - daily-start.md / daily-wrapup.md / fallback-instruction.ts を mt CLI ベースに書換
  - 旧 schema 厳守ルール / AI 回答ルール 段落削除 (API 層で強制されるので不要)
  - daily-start.md / daily-wrapup.md / fallback-instruction.ts を mt CLI 呼び出しに書換
- [ ] **BL-TBD-006 Phase 3c: cron + 承認後自動更新** — 未着手 (Phase 4 完了後、`mt morning finalize` で簡素実装可能になる)
- [ ] **BL-TBD-007 Phase 3d: 「📅 今日の運用」UI 拡張 + 「📚 過去の運用」リンク** — 未着手 (Phase 4 完了後)
