...
> 詳細は本ドキュメントの末尾「10. 関連ドキュメント」を参照。設計監査と MCP/skill 起動手順は隣の [INTEGRATIONS.md](INTEGRATIONS.md)。
>
> Last updated: 2026-05-05 (Phase 5 完了 + 整合性 hotfix 24 件全消化)
...
| 常駐 | launchd service `jp.mini-tachyon` |
...
```bash
# launchd 状態 (state / pid / last exit code を 1 行で)
...
# CLI で smoke test
~/mini-tachyon/bin/mt projects list --human
```

落ちている時は `~/mini-tachyon/launchd/install.sh restart` (内部で `launchctl kickstart -k gui/$(id -u)/jp.mini-tachyon`、未ロード時は install)。
初回セットアップ / plist 入れ替え時は `~/mini-tachyon/launchd/install.sh install` (旧 label `jp.agi-tools.mini-tachyon` の残骸があれば自動 bootout)。
...
2. ミニタキオンが Cockpit task を spawn (固定プロンプト `~/mini-tachyon/lib/prompts/daily-start.md`)
3. 朝のエージェントが:
...
| マーク | action | 起こること |
|---|---|---|
| `[x]` | approve | BL.state を `in_progress` に上げる。さらに **「分類」列が 🟢 A** なら **cockpit task を新規 spawn** し、その task ID を BL.cockpit_task_ids に追加する。AI がそのまま手を動かし始める |
| `[-]` | defer | BL.state を `planned` に (=「今日はやらない、後回し」)。cockpit task は作らない |
| `[ ]` | skip | 状態を変えず、何もしない (案として提示されただけで見送る) |

#### 軸 2. 分類 (朝のエージェントが書く)

AI が朝整理時にテーブルの「分類」列に **🟢 A / 🟡 B / 🔴 C** のいずれかを書く (固定プロンプト `~/mini-tachyon/lib/prompts/daily-start.md` で定義)。

| 分類 | 意味 | `[x]` 採択時の挙動 |
|---|---|---|
| 🟢 **A** | **AI で即実行可** (well-specified、要件が揃っていて並列起動できる) | cockpit task を **auto-spawn**。AI が claude エージェントとして自走を始め、完了したら `mt deliverable register` で成果物登録 |
| 🟡 **B** | **要件詰めれば A 化** (まず田中さんとヒアリングが必要、そのあと A に昇格) | spawn しない。BL.state だけ `in_progress` に上げる。次の往復で AI が田中さんに `mt bl add-question` で確認を投げる流れ |
| 🔴 **C** | **本人実行** (ハローワークに行く / 印鑑を押す等、AI ではできない作業) | spawn しない。BL.state だけ `in_progress` に上げる。田中さんが自分で進める |

#### 軸 1 × 軸 2 のクロス表

| | 🟢 A | 🟡 B | 🔴 C |
|---|---|---|---|
| `[x]` approve | `in_progress` + cockpit spawn (BL.cockpit_task_ids に append) | `in_progress` のみ | `in_progress` のみ |
| `[-]` defer | `planned` (今日見送り) | `planned` | `planned` |
| `[ ]` skip | 変化なし | 変化なし | 変化なし |

> **「`spawn_class: A` のフラグはどういう時に立つ?」の答え**:
> 朝のエージェントが「分類」列に 🟢 A と書いた行に対して **田中さんが `[x]` を付けた時だけ**。
> 内部的には `mt morning finalize` (フィードバック処理エージェントが実行) の `selections[]` JSON に `{ "bl": "BL-XXXX", "action": "approve", "spawn_class": "A", "next_action": "..." }` という形で渡される。サーバが `spawn_class === "A"` を見て cockpit task を起動する判断をする (`lib/api/finalize.ts:86`)。
>
> 言い換えると: **「AI に今すぐ自走で着手させたい BL」のフラグ**。
> B/C は人間判断・人間作業が挟まるので spawn しない。

#### `✍️ 回答` 欄

タスク案ファイル末尾の Q1/Q2/... の `✍️ 回答:` 欄に田中さんが書き込んだ文字列は、フィードバック処理エージェントが `mt bl add-decision --type answer --by user --content "..." --answers-question-id <qid>` で各 BL の `decisions[]` に分解 append する。

#### 例 (実際の selections JSON)

朝のテーブルが:
```
| BL-0073 | 章立て作成 | 5/10 | 🟢 A | (略) | 章立て 5 章で着手 | [x] |
| BL-0070 | 図版調達     | 5/12 | 🟡 B | (略) | 章数決まったら   | [-] |
| BL-0011 | ハロワ提出  | 5/8  | 🔴 C | (略) | 田中さんが行く   | [x] |
```

→ サーバに送られる JSON は:
```json
[
  { "bl": "BL-0073", "action": "approve", "spawn_class": "A", "next_action": "章立て 5 章で着手" },
  { "bl": "BL-0070", "action": "defer" },
  { "bl": "BL-0011", "action": "approve", "spawn_class": "C", "next_action": "田中さんが行く" }
]
```

→ 結果:
- BL-0073: `state=in_progress` + cockpit task spawn (新エージェントが章立て作成を開始)
- BL-0070: `state=planned` (defer、cockpit 作らない)
- BL-0011: `state=in_progress` (cockpit 作らない、田中さんがハローワークに行くタスク)
...
仕様上は「`☀️` を 8:30 までに押し忘れ → ホームに警告バッジ (自動実行はしない)」だが、**警告バッジは Phase 6+ 実装予定** (2026-05-05 時点 spec only、rule 13 §3.4)。現状は気付いたタイミングで押せば OK。

---

## 6. 夜の運用 (🌙 daily wrapup)

iPhone でホームから **🌙 今日を締める** を押下。固定プロンプト `~/mini-tachyon/lib/prompts/daily-wrapup.md` 通りに:
...
~/mini-tachyon/bin/mt deliverable register \
  --bl BL-XXXX \
  --file <basename>.md \
  --title "..." \
  --agent claude \
  --task <この cockpit task id>           # 任意
  # --created-by <label>                  # 任意 (default: agent 名で自動)
  # --id d-20260505-001                   # 任意 (省略時は d-<YYYYMMDD>-<NNN> で自動採番)
  # --date 2026-05-05                     # 任意 (default: 今日)
```

register 直後の `review_state` は `unreviewed`。MCP 経由なら `mt_deliverable_register` ツールに同等の引数。
...
- `deliverable.review_log[].by` (add-comment 経由) は `user | ai` のみ (2026-05-05 から zod enum で強制)
...
| 症状 | 対応 |
|---|---|
| `mt: command not found` | `~/mini-tachyon/bin/mt` を絶対パスで使う、または `sudo ln -s ~/mini-tachyon/bin/mt /usr/local/bin/mt` |
| UI に書いたはずの変更が反映されない | (1) chokidar 取りこぼし → 5 分待つと fallback rebuild、(2) スキーマ違反で reject されている → `mt bl get <id>` で実際の状態確認 |
| `error: network` | mini-tachyon 落ちてる → `~/mini-tachyon/launchd/install.sh status` 確認、`~/mini-tachyon/launchd/install.sh restart` で再起動 |
...
| `error: cockpit` (502) | cockpit binary 経路で失敗 → `detail.attempts[]` で send/resume/create のどこで死んだか確認、cockpit が立ってるか `cockpit task list`。default パス上書きは `COCKPIT_BIN` 環境変数 |
...
| iPhone から繋がらない | iPhone が Tailscale にログインしているか確認。Mac mini 側 `~/mini-tachyon/launchd/install.sh status` でポート 3000 が listening になっているか |
| `launchctl list jp.agi-tools.mini-tachyon` が空 | 旧 label。**現行 label は `jp.mini-tachyon`** (2026-05-02 に独立化、2026-05-05 に install.sh 統一)。旧残骸は `install.sh install` で自動 bootout される |
...
| 種別 | パス |
|---|---|
| **設計監査 + 統合手順** | [INTEGRATIONS.md](INTEGRATIONS.md) (隣のファイル、24 件 hotfix 完了記録 + 4 client 別 MCP 起動手順 + Part 4「Claude Code 等がルールを認識する 5 経路」) |
| 運用ルール (canonical) | `~/aipm_v0/.cursor/rules/aios/ops/13_mini_tachyon_protocol.mdc` |
| プロジェクト STATUS | `~/aipm_v0/Stock/作業効率化/ミニタキオン/STATUS.md` |
| 設計書 (オリジナル) | `~/aipm_v0/Flow/202604/2026-04-25/ミニタキオン/design_mobile_review_hub.md` |
| インフラ独立化記録 (2026-05-02) | `~/aipm_v0/Stock/作業効率化/ミニタキオン/インフラ独立化とTailscale接続問題_2026-05-02.md` |
| CLI 仕様 | `~/mini-tachyon/docs/CLI.md` |
| HTTP API 仕様 | `~/mini-tachyon/docs/API.md` |
| MCP server 仕様 | `~/mini-tachyon/docs/MCP.md` |
| MCP config 例 (client 別 3 種) | `~/mini-tachyon/examples/` (`claude-code-mcp.sample.json` / `claude-desktop-mcp.sample.json` / `cursor-mcp.sample.json` + README) |
| Claude Code skill | `~/.claude/skills/mini-tachyon/SKILL.md` (`/mini-tachyon` で発火) |
...
困ったらまず `13_mini_tachyon_protocol.mdc` を読む。コマンドのリファレンスは `docs/CLI.md`、API スキーマ詳細は `docs/API.md`、MCP tool 一覧と起動手順は `docs/MCP.md` + 隣の `INTEGRATIONS.md` Part 3。
