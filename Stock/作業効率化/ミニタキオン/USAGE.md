...
落ちている時は `launchctl kickstart -k gui/$(id -u)/jp.mini-tachyon` で再起動。
...
| `error: network` | mini-tachyon 落ちてる → `launchctl list jp.mini-tachyon` 確認、`launchctl kickstart -k gui/$(id -u)/jp.mini-tachyon` で再起動 |
